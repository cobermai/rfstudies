...
class ExplainerCreator(ABC):
    ...
    @staticmethod
    @abstractmethod
    def get_sample_importance(
            explainer_model: typing.Any,
...
def explain_samples(
        explainer: ExplainerCreator, model: Model, X_reference: np.ndarray,
    ...
    """
    built_explainer = explainer.build_explainer(model=model,
                                                X_reference=X_reference)
    sample_importance = explainer.get_sample_importance(
        explainer_model=built_explainer, data_to_explain=X_to_explain)
