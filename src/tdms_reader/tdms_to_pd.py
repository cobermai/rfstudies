...
def path_to_pd(path: str,
               ...
               grp_of_interest: list = None) -> pd.DataFrame:
    """
    ...
    :param format: The format the pandas dataframe should have.
    ...
    if os.path.isfile(path):
        with nptdms.TdmsFile.read(path) as tdmsfile:
            return tdmsfile_to_pd(tdms = tdmsfile,
                                  format = format,
                                  ch_of_interest = {ch.name for ch in tdmsfile.groups()[0].channels()} if ch_of_interest == None else set(ch_of_interest),
                                  grp_of_interest = {grp.name for grp in tdmsfile.groups()} if grp_of_interest == None else set(grp_of_interest))
    else:
...
def tdmsfile_to_pd(tdms:nptdms.TdmsFile,
    ...
    """
    This function transforms a tdmsfile into a pandas data frame. The output channels can be restricted to the channels of
    interest.
    "matrix_of_vectors": no further requirements to the tdms file format.
        In each cell of the dataframe a vector with the channel data is placed.
    "matrix_of_scalars": requires all channels to have the same length.
    ...
    :param format: The format the pandas dataframe should have. One of {"matrix_of_vectors", "matrix_of_scalars"}
    :param ch_of_interest: The channels that should be included in the channels of interest.
    ...
    if format == "matrix_of_scalars":  # in xbox, use this for TrendData
        ...
        for grp_name in grp_of_interest:
            if 0 == sum([len(ch.data) for ch in tdms[grp_name].channels()]):
                ...
                df.loc[[tdms[grp_name].name], actual_channels] = [np.NaN for ch in actual_channels]
            else:
                actual_channels = list(ch_of_interest.intersection({ch.name for ch in tdms[grp_name].channels()}))
...
def tdmsgroup_to_pd(grp:nptdms.TdmsGroup,
    ...
    """
    This function transforms a tdms group into a pandas data frame. The output channels can be restricted to the channels of
    interest.
    ...
    if test_interests and ch_of_interest != ch_of_interest.intersection({ch.name for ch in grp.channels()}):
        ...
        print("The channels of interreset above do not all occure in " + grp.name +". The none existing ones are discarted.")
