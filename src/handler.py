...
class XBox2ContextDataCreator(ContextDataCreator):
    ...
    def manage_features(self):
        ...
        feature_list = list(
            get_event_attribute_features(length=self.num_events))
        self.manage_event_attribute_features(feature_list)

        feature_list = list(
            get_trend_data_features(length=self.num_events,
                                    trend_data_file_path=self.td_file_path))
        self.manage_trend_data_features(feature_list)

        feature_list = list(get_event_data_features(length=self.num_events))
        self.manage_event_data_and_tsfresh_features(
            event_data_features=feature_list)
    ...
    def manage_event_attribute_features(self, features: typing.List) -> None:
        ...
        column_wise_handler = ColumnWiseContextDataWriter(
            self.dest_file_path, length=self.num_events)
        for feature in features:
    ...
    def manage_trend_data_features(self, features: typing.List) -> None:
        ...
        cw_handler = ColumnWiseContextDataWriter(self.dest_file_path,
                                                 length=self.num_events)
        for feature in features:
            feature.vec = feature.calculate_all(loc)
            cw_handler.write_column(feature)

    def manage_event_data_and_tsfresh_features(
            self, event_data_features: typing.List) -> None:
        ...
        row_wise_handler = RowWiseContextDataWriter(self.dest_file_path,
                                                    length=self.num_events)
        with h5py.File(self.ed_file_path, "r") as file:
            data_gen = ({key: channel[:]
                         for key, channel in grp.items()}
            ...
            for data, index in tqdm(zip(data_gen, itertools.count(0))):
                ...
                row_wise_handler.write_row_from_external(index=index,
                                                         data_iter=val_gen)
    ...
    def feature_post_processing(self):
        ...
        with h5py.File(self.dest_file_path, "r+") as file:
            ...
            file.create_dataset(name="is_bd_in_20ms",
                                data=np.append(is_bd[1:], [False]))
            file.create_dataset(name="is_bd_in_40ms",
                                data=np.append(is_bd[2:], [False, False]))
            file.create_dataset(name="is_healthy",
                                data=file["clic_label/is_healthy"])
            ...
            file.create_dataset(name="event_ext_link_index",
                                data=get_event_timestamp_ext_link_index(
                                    ext_link_file=ext_link_file,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="for xbox2 dataset: extract features for ml")
    parser.add_argument(
        "td",
        type=Path,
        help=
        "file path of an hdf file where all TrendData is merged into one dataset"
        "for each signal.")
    parser.add_argument(
        "ed",
        type=Path,
        help=
        "file path of an hdf file where all EventData groups are gathered (e.g."
        "with ExternalLinks).")
    parser.add_argument(
        "dest",
        type=Path,
        help="file path of the hdf file where the features will be written.")
