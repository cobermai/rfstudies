...
import h5py
from context_data_creator import Feature


def get_features() -> typing.Generator:
    for is_type in ["is_log", "is_bdin40ms", "is_bdin20ms", "is_bd"]:
    ...
    for chn in ['PEI Amplitude', 'PKI Amplitude', 'PSI Amplitude', 'PSR Amplitude']:
        yield Feature(name="pulse_length", func=func_len, output_dtype=float, hdf_path=chn, info=func_len.__doc__)
        yield Feature(name="pulse_amplitude", func=func_amp, output_dtype=float, hdf_path=chn, info=func_amp.__doc__)

    for chn in ['DC Up', 'DC Down']:
        yield Feature(name="min", func=apply_func_creator(np.min), output_dtype=float, hdf_path=chn,
                      info="calculates the minimum of the data")
        yield Feature(name="D1", func=apply_func_creator(partial(np.quantile, q=.1)), output_dtype=float, hdf_path=chn,
                      info="calculates the first decile of the data")
        yield Feature(name="mean", func=apply_func_creator(np.mean), output_dtype=float, hdf_path=chn,
                      info="calculates the mean of the data")
        yield Feature(name="D9", func=apply_func_creator(partial(np.quantile, q=.9)), output_dtype=float, hdf_path=chn,
                      info="calculates the 9th decile of the data")
        yield Feature(name="max", func=apply_func_creator(np.max), output_dtype=float, hdf_path=chn,
...
def log_type_translator(is_type: str) -> typing.Callable[[Path, str], bool]:
    ...
    :param is_type: string of the type that were interested in (in {"is_log", "is_bdin40ms", "is_bdin20ms", "is_bd"})
    """
...
def pulse_amplitude(file_path: Path, hdf_path: str) -> float:
    """calculates the mean value where the amplitude is higher than the threashold (=half of the maximal value)."""
    with h5py.File(file_path, "r") as file:
        data: npt.ArrayLike = file[hdf_path][:]
        threshold: float = data.max() / 2
